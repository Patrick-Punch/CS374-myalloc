...
void initialize_heap(){
    // Initialize the heap with a block large enough to hold the memory
    void *heap = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    head = (struct block *)heap;
    head->size = 1024 - sizeof(struct block); 
    head->in_use = 0;
    head->next = NULL;
}

void *myalloc(int size){
    // Cannot allocate more than 1008 bytes
    if (size > 1008){
        return NULL;
    }
    ...
    while (cur != NULL){
        if (cur->in_use == 0 && cur->size >= size){
            ...
            // Allocated block size accounts for padding
            if ((unsigned long)cur->size > size + sizeof(struct block)){
                return (void *)((char *)cur + sizeof(struct block));
            }
            return (void *)((char *)cur + sizeof(struct block)); // Return pointer to data
        }
        cur = cur->next;
    }
